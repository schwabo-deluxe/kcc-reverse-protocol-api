...
| `UtilizationRateMinutes` | Trailing-Fenster (Standard `15`), aus dem UPH und Prozent hochgerechnet werden. Klein = reagiert sofort auf kurze Stöße; groß = geglättet. `Count` und der Verlauf bleiben über das ganze Fenster. |
...
| `GET /auslastung` | Auslastung der Ressourcenpunkte aus `TSPORD`-Telegrammen (UPH, % vom Richtwert, Verlauf je Punkt), gebündelt nach `Group`. Bei Kacheln mit `Connection` (RBG) sind **Tacho und Linienchart die RBG-Spielauswertung** (Leistung gegen `RbgMaxCyclesPerHour`, Spiele/h-Verlauf), die `TSPORD`-Zähler dienen nur der Ziel-Identifikation; darunter zeitbasierte Auslastung, Doppel-/Einzelspiele, Leerlaufzeit, Ø Auftragsdauer ein/aus |
