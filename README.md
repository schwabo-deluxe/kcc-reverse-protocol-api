...
| `RbgMaxCyclesPerHour` | Vorgabe für die max. Vollspiele/h eines RBG (Standard `60`). Basis der RBG-Auslastung: `(Vollspiele + Halbspiele/2) / (max · Stunden)`. Je Punkt über `ResourcePoints[].MaxCyclesPerHour` überschreibbar. |
| `RbgPutDoneCodes` / `RbgFetchDoneCodes` | MessageCodes einer abgeschlossenen Ein- bzw. Auslagerung. Leer ⇒ `["ENDDEP"]` / `["ENDPUP"]`. Diese Ende-Telegramme zählen für Voll-/Halbspiele und die Auslastung. |
...
| `GET /auslastung` | Auslastung der Ressourcenpunkte aus `TSPORD`-Telegrammen (UPH, % vom Richtwert, Verlauf je Punkt), gebündelt nach `Group`. Kacheln mit `Connection` zeigen zusätzlich die RBG-Spielauswertung: Voll-/Halbspiele, Auslastung gegen `RbgMaxCyclesPerHour`, Leerlaufzeit, Ø Auftragsdauer ein/aus |
