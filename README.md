...
| `RbgPickOrderCodes` / `RbgPickDoneCodes` / `RbgDropOrderCodes` / `RbgDropDoneCodes` | MessageCodes eines RBG-Transports. Leer ⇒ `["PUPORD"]` / `["ENDPUP"]` / `["DEPORD"]` / `["ENDDEP"]`. **Wichtig:** die Codes heißen *aufnehmen* und *abgeben*, nicht *einlagern* und *auslagern* — jeder Transport besteht aus beidem. `ENDDEP` beendet einen Transport und wird gezählt; `PUPORD` markiert seinen Beginn für die Dauermessung. |
