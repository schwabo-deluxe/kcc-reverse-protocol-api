...
| `ConveyorOrderCodes` / `ConveyorEndCodes` / `ConveyorFreeCodes` | MessageCodes der Fördertechnik-Belegung. Leer ⇒ `["TSPORD"]` / `["ENDTSP"]` / `["RPFREE"]`. `TSPORD` = Transportauftrag erteilt, `ENDTSP` = Transport beendet (der Punkt wartet auf den nächsten Auftrag), `RPFREE` = Ressourcenpunkt meldet sich frei. Daraus: **Belegung** = Σ(`TSPORD`→`ENDTSP`) ÷ Fenster, dazu Ø Transportdauer und Ø Wartezeit bis zum nächsten Auftrag. Anders als beim RBG laufen die Transporte eines Punkts nacheinander — der Wert ist deshalb ein echtes Zeitmaß ohne Doppelzählung. |
...
| `GET /auslastung` | Auslastung der Ressourcenpunkte, gebündelt nach `Group`. Jede Kachel zeigt **zwei Tachos — Zeitseite und Mengenseite**. Punkte mit `Connection` (RBG): **Auslastung** (Zeit mit offenem Auftrag) und **Leistung** (Spiele/h gegen `RbgMaxCyclesPerHour`); Tacho, Linienchart und Kopfzeile rechnen dort in Doppel-/Einzelspielen, die `TSPORD`-Zähler dienen nur der Ziel-Identifikation. Punkte ohne `Connection` (Fördertechnik): **Belegung** (`TSPORD`→`ENDTSP` ÷ Fenster) und **Leistung** (% vom UPH-Richtwert), darunter Ø Transport- und Wartezeit. Alle Kennzahlen tragen einen Erklär-Tooltip; die Tachos blenden zwischen zwei Abrufen über, statt zu springen |
