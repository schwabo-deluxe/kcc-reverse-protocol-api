...
| `RbgPutOrderCodes` / `RbgGetOrderCodes` | MessageCodes der Auftragserteilung. Leer ⇒ `["DEPORD"]` / `["PUPORD"]`. Auftrag→Ende-Paare (per `ResourceLabel`) liefern die Ø Ausführungsdauer und die Leerlaufzeit. Die Anlage doppelt jedes Ereignis (`DM`/`AK`); das wird zusammengeführt. |
